...
We use the [SnapshotTesting](https://github.com/pointfreeco/swift-snapshot-testing) library for our snapshot tests. Please note that snapshot results can differ depending on whether the tests are recorded on an Apple Silicon or Intel processor. For consistency, all reference snapshots should be recorded using an Apple Silicon machine.
