...
public class BottomSheetView: UIView {
	...
	private var contentSelected: [String: String] = [:]
	public var dismissBottomSheet: ((BottomSheetSelectionResponse) -> Void)?
	private var isDismissing: Bool = false
	private var bottomSheetSelectionResponse: BottomSheetSelectionResponse
	...
	public init(configuration: BottomSheetConfiguration) {
		...
		bottomSheetSelectionResponse = .init(action: .dismiss,
												  content: config.content.reduce([:]) {
							 var dict: [String : String] = [:]
							 dict[$1.id] = $1.selectedId ?? $1.items.first?.id
							 return dict
						 })
...
private extension BottomSheetView {
	func setupView() {
		...
		for content in config.content {
			...
			for item in content.items {
				...
				let itemTapGesture = ItemInformationTapGesture(target: self, action: #selector(self.selectItem(_:)))
		...
		NSLayoutConstraint.activate([
			...
			itemsStackView.topAnchor.constraint(equalTo: descriptionLabel.bottomAnchor, constant: 16.0),
	...
	@objc private func selectItem(_ sender: ItemInformationTapGesture) {
		if let content = contentInfo[sender.contentId], let item = content[sender.itemId] {
			contentSelected[sender.contentId] = sender.itemId
			content.filter { $0.key != sender.itemId }.forEach { $1.isActivated = false }
			item.isActivated = true
			if !isDismissing && sender.autoSubmit {
				isDismissing = true
				self.bottomSheetSelectionResponse.action = .submit
				self.bottomSheetSelectionResponse.content = contentSelected
				DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
					self.dismissBottomSheet?(self.bottomSheetSelectionResponse)
				}
			}
		}
	}
